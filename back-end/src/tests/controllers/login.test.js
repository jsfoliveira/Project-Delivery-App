...
describe('Testes do User', () => {
  ...
  before(async () => {
    ...
    sinon.stub(userService,'createAdm').onCall(0).resolves(mockCreateService2)
      .onCall(1).resolves({ message: 'Unauthorized user!!!' });
  ...
  describe('Teste do UserController', () => {
    ...
    //     req.headers = { authorization: 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJuYW1lIjoiQ2xpZW50ZSBaw6kgQmlyaXRhIiwiZW1haWwiOiJ6ZWJpcml0YUBlbWFpbC5jb20iLCJyb2xlIjoiY3VzdG9tZXIiLCJpYXQiOjE2NjQ0ODg0MTl9.O5mdESsEJ9EbjqgXwtxxOglFGDCpaGkdD5KHPF-QXJI'}
    //     await userController.createAdm(req, res);
    //     console.log(res.status);
    //     expect((res.status).calledWith(401)).to.be.true;
    //     expect((res.json).calledWith({ message: 'Unauthorized user!!!' })).to.be.true;
    //   });
    // })
